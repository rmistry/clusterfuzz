...
class Issue(issue_tracker.Issue):
    ...
    def __init__(self, data, is_new, tracker):
        ...
        labels = [
            str(hotlist_id) for hotlist_id in data['issueState'].get('hotlistIds', [])
        ]
    ...
    def body(self):
        ...
        if not self._is_new:
            ...
            result = self.issue_tracker._execute(
                self.issue_tracker.client.issues()
                .issueUpdates()
                .list(issueId=str(self.id), pageSize=1, sortBy='ASC')
            )
    ...
    def actions(self):
        ...
        while True:
            result = self.issue_tracker._execute(
                self.issue_tracker.client.issues()
                .issueUpdates()
                .list(issueId=str(self.id), pageToken=page_token, sortBy='ASC')
            )
    ...
    def _add_update_single(
        self, update_body, added, removed, field_name, api_field_name, modifier=None
    ):
    ...
    def _update_issue(self, new_comment=None, notify=True):
        ...
        self._add_update_single(
            update_body, added, removed, 'assignee', 'assignee', _make_user
        )
        self._add_update_single(
            update_body, added, removed, '_verifier', 'verifier', _make_user
        )
        self._add_update_single(
            update_body, added, removed, 'reporter', 'reporter', _make_user
        )
        self._add_update_single(update_body, added, removed, 'title', 'title')
        self._add_update_collection(
            update_body, added, removed, 'ccs', 'ccs', _make_users
        )
        ...
        for hotlist in removed_hotlists:
            self.issue_tracker._execute(
                self.issue_tracker.client.hotlists()
                .entries()
                .delete(hotlistId=str(hotlist), issueId=str(self.id))
            )
        return result
    ...
    def save(self, new_comment=None, notify=True):
        ...
        if self._is_new:
            ...
            print(dir(self.issue_tracker))
            print(dir(self.issue_tracker.client))
            print('NEW ONE')
            print(dir(client.build()))
            result = self.issue_tracker._execute(
...
class Action(issue_tracker.Action):
    ...
    def _get_field_update(self, field_name):
        ...
        return next(
            (
                update
                for update in self._data['fieldUpdates']
...
# HERE HERE - IssueTracker is here.
class IssueTracker(issue_tracker.IssueTracker):
    ...
    def _execute(self, request):
        ...
        for _ in range(2):
            try:
                # HERE HERE
                print('ABOUT TO EXEWCUTE THE REQUEST!')
                print(dir(request))
    ...
    def find_issues(self, keywords=None, only_open=None):
        ...
        page_token = None
        while True:
            issues = self._execute(
                self.client.issues().list(
...
if __name__ == '__main__':
    # TODO(rmistry): Run how to do stuff...
    config = {'default_component_id': 1363359}
    # http_client can be None. Maybe useful for testing purposes?
    # Not sure what the project_name is used for..
    it = IssueTracker('chromium', None, config)
    issue = it.new_issue()
